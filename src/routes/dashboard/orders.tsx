  ...
  return (
    <div className="space-y-8">
      ...
      <Tabs defaultValue="all" className="w-full" onValueChange={setActiveTab}>
        <TabsList className="bg-white p-1 rounded-2xl border border-border h-auto overflow-x-auto flex-nowrap w-full lg:w-auto">
          <TabsTrigger value="all" className="rounded-xl px-6 py-2.5 data-[state=active]:bg-primary data-[state=active]:text-white">Total</TabsTrigger>
          <TabsTrigger value="pending" className="rounded-xl px-6 py-2.5 data-[state=active]:bg-primary data-[state=active]:text-white">Pending</TabsTrigger>
          <TabsTrigger value="delivered" className="rounded-xl px-6 py-2.5 data-[state=active]:bg-primary data-[state=active]:text-white">Delivered</TabsTrigger>
          <TabsTrigger value="cancelled" className="rounded-xl px-6 py-2.5 data-[state=active]:bg-primary data-[state=active]:text-white">Cancelled</TabsTrigger>
        </TabsList>
                  ...
                  <Card key={order.id} className="border-none shadow-soft hover:shadow-elegant transition-all group overflow-hidden">
                    <CardContent className="p-0">
                      <div className="flex flex-col md:flex-row">
                        {/* Image */}
                        <div className="w-full md:w-48 h-48 md:h-auto overflow-hidden bg-slate-50 border-r border-border/50">
                          <img 
                        ...
                        <div className="flex-1 p-6 space-y-4">
                          <div className="flex flex-wrap justify-between items-start gap-4">
                            <div>
                              <p className="text-xs font-bold text-muted-foreground tracking-widest uppercase mb-1">{order.id}</p>
                              <h3 className="text-lg font-bold group-hover:text-primary transition-colors">{order.productName}</h3>
                            </div>
                            <Badge variant="outline" className={`px-4 py-1.5 rounded-full font-bold border-2 ${getStatusColor(order.deliveryStatus)}`}>
                          ...
                          <div className="flex justify-end">
                            <Button variant="ghost" className="rounded-full hover:bg-primary/5 hover:text-primary gap-2">
                              View Details
